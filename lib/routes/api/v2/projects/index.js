...
var createProject = function(req, res, next){
  ...
  for( var field, length, idx = 0; idx < detailFields.length; idx ++ ) {
    field = detailFields[idx];
    length = (project[field[0]] || "").length;
    if(
    length === 0 || /* empty */
    (field[1] === 1 && (length < 80 || length > 120)) || /* for description */
    (field[1] === 2 && (length < 200 || length > 500))) /* for longer question */ {
      return res.status(500).json({ error: field[0] + "_required" });
    }
  }
...
var updateProject = function(req, res, next) {
  ...
  project.revisions.push({description: project.description, timestamp: Date.now()});
