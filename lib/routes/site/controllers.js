...
import {port, host, live, discourseUrl, disqus_shortname, googleAnalytics, facebookAppId, grantdash} from 'config';
...
export const viewsStack = [
  setViewVar('grantdash', grantdash),
...
export const projectStack = []
  ...
  .concat([function(req, res, next) {
    if (req.project) {
      res.locals.projectId = req.project._id;
    }
    res.locals.proto = req.protocol;
    next();
  }])
  .concat([render('project')]);
